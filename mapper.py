        ...
        for domain in domains:
            ...
            for res_key in resDict.keys():  # iterate on resource dictionary keys
                ...
                for dk in domain_keys:
                    ...
                    dk = dk.encode().decode('utf-8')  # make sure utf-8 mismatches don't skip section
                    if not mapped and re.search(dk, res_key, re.IGNORECASE):
                        print("domain key is " + dk + "section name is" + res_key)
                        try:
                            if not is_custom_map_fn:
                                # use the pre-defined mapper functions
                                mapper = "map_" + domain.lower() + "(resDict[res_key], res_key, db_res, lang, g, 0)"
                                res_elems += eval(
                                    mapper)  # calls the proper mapping for that domain and counts extracted elements
                                mapped = True  # prevents the same section to be mapped again
                            else:
                                mapper = map_user_defined_mappings(domain, resDict[res_key], res_key, db_res, lang, g,
                                                                   0)
                                res_elems += mapper  # calls the proper mapping for that domain and counts extracted
                                # elements
                                mapped = True  # prevents the same section to be mapped again
                        except SyntaxError:
    ...
    for elem in elem_list:
        if type(elem) == list:  # for nested lists (recursively call this function)
            ...
            if ref:  # current element contains a reference
                ...
                if uri_name == None: uri_name = general_mapper(
                    elem)  # no reference found, try general mapping (less accurate
                if (uri_name and uri_name != "" and uri_name != res):
                    uri_name = uri_name.replace(' ', '_')
                    uri_name = urllib.request.quote(uri_name)
                    uri = dbr + uri_name
        ...
        if type(elem) == list:  # for nested lists (recursively call this function)
            ...
            if uri and uri != "":
                if len(list(g.triples((rdflib.URIRef(uri), dbo.alumni, res)))) == 0 and \
                        len(list(
                            g.triples((rdflib.URIRef(uri), dbo.academicDiscipline, res)))) == 0:  # if already mapped
        ...
        if type(elem) == list:  # for nested lists (recursively call this function)
            ...
            if res_name:
                ...
                if ref:  # current element contains a reference
                    ...
                    uri_name = quote_mapper(elem)  # try finding awards in quotes
                    if uri_name == None or uri_name == res: uri_name = general_mapper(
                        elem)  # no reference found, try general mapping (less accurate
                    if (uri_name and uri_name != "" and uri_name != res):
                        uri_name = uri_name.replace(' ', '_')
                        uri_name = urllib.request.quote(uri_name)
...
def map_contributors(elem_list, sect_name, res, lang, g, elems):
    ...
    for elem in elem_list:
        if type(elem) == list:  # for nested lists (recursively call this function)
            ...
            try:
                subsection = sect_name.split('-')[1].strip()
            except:
                pass
...
def wikidataAPI_call(res, lang):
    ...
    # print(res)
    enc_res = urllib.parse.quote(res)  # then encode the string to be used in a URL
    ...
    except urllib.error.URLError as e:  # sometimes the host can refuse too many connections and returns a socket error
        print(e.reason)
