...
def main_parser(language, resource):
    ...
    if not result:  # if the result is empty, try again looking for page redirects
    ...
    cleanlists = utilities.clean_dictionary(language,
                                            lists)  # clean resulting dictionary and leave only meaningful keys
...
def parse_list(list_elem):
    ...
    if 'content' in list_elem and list_elem['content'] is not None:
...
def find_page_redirects(res, lang):
   ...
   """
    try:
        ...
        pipe_output = proc.stdout.read()  # redirect the input into python variable
        proc.kill()  # kill the spawned process
        result = json.loads(pipe_output.decode("utf-8", "ignore"))  # load the string as a python dict
